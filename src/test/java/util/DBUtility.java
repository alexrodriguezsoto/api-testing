...
public class DBUtility {
    ...
    public static void establishConnection(Database dbType, String database) {
        try {
            switch (dbType) {
                case POSTGRESQL:
                    connection = DriverManager.getConnection(ConfigurationReader.readProperty(database, propertyPath),
                            ConfigurationReader.readProperty("dbUserName", propertyPath),
                            ConfigurationReader.readProperty("dbPassword", propertyPath));
                    if (connection != null) {
